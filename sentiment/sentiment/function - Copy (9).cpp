...
void initFormatted(vector<wordStruct> original, vector<wordStruct>& formatted) {
...
void lowercaseReview(vector<wordStruct> originalReview, vector<wordStruct>& formattedReview) { // this formatted review will be passed into all the other functions
...
//, vector<int>& universalChangedIndex
void reviewSentiment(vector<wordStruct> formattedReview) {
    for (auto i : formattedReview) {
        wordStruct currentWord = i;

        int index = binarySearch(currentWord.word, formattedReview, formattedReview.size(), 0); // finds index of currentWord in sentiment.txt 

        cout << "\n+++++++++++++before adding value: " << currentWord.value;
        
        if (index != -1) { // IF BINARY SEARCH DOESNT FAIL
            //universalChangedIndex.push_back(index);  // will be used in printPos() & printNeg()
            cout << "\n+++++++++++++before adding value: " << currentWord.value;
        }
...
void printPos(vector<wordStruct> formattedReview, vector<wordStruct> originalReview, vector<int>& changedIndex) { // prints the words updated to be more negative
...
// ------ ------ ------ ------ ------ ------ ------ ------ 
// need to: "bool needsChange, if true then go through but if no change needed, then cout "review not updated"
void replacePos(vector<wordStruct> formattedReview, vector<wordStruct> originalReview, vector<wordStruct> negList, vector<int>& changedIndex) { // replaces the words that need to be more negative
    ...
    for (int i = 0; i < formattedReview.size(); i++) { // for each in review, checks if the sentiment value is above 1.
        if (formattedReview[i].value > 1) {
            ...
            changedIndex.push_back(i); // adds the index of the replaced word to a vector
        } // end if
    ...
    if (!needsChange) { // if does not need change
        ...
        printPos(formattedReview, originalReview, changedIndex); // prints the words changed from pos to negative
    ...
    // NOTE :::: NEEDS TO CHECK IF THE UNIVERSAL CHANGED INDEX IS POS OR NEG
    // BINARY SEARCH ORIGINAL REVIEW AT I IF CHANGED INDEX[I] == I 
}
...
void replace(vector<wordStruct> review) { // parent replace. will call the replace neg, and replace pos, respectively.  
    // call binary search?
}
